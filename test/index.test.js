...
describe('html-tokenizer', () => {
  [
    ...
    (item.only?it.only:it)('should tokenize ' + JSON.stringify(item.html) + (item.entities?' with entities':''), () => {
  ...
  it('should parse more entities in attributes', () => {
    const tkzr = new Tokenizer({ entities: entityMap });
    const [tkn] = [...tkzr.tokenize('<x y="&deg;"/>')]
      .filter(t => t.type === 'attribute');
    assert.strictEqual(tkn.value, '\u00B0');
  });
  ...
  describe('parser', () => {
    ...
    [{html: '', events: 'start,done'},
      ...
      (item.only?it.only:it)('should parse '+JSON.stringify(item.html), () => {
    ...
    it('should parse a wikipedia page', () => {
      ...
      const page = fs.readFileSync(__dirname + '/data/wikipedia.html', 'utf8');
      const parser = new Parser();
      let content = [];

      for (const { type, text, name, attributes, selfClosing } of parser.parse(page)) {
        if (type === 'open') {
          content.push(selfClosing
            ? `<${name}${attify(attributes)}/>`
            : `<${name}${attify(attributes)}>`
          );
        } else if (type === 'close' && !selfClosing) {
          content.push(`</${name}>`);
        } else if (type === 'text') {
          content.push(text);
